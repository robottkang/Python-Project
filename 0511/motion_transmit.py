...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1440)      # 프레임 폭을 480으로 설정 
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 960)     # 프레임 높이를 320으로 설정
i = 0
j = 0
if cap.isOpened():
...
##================================================================
        ...
        if diff_cnt > max_diff:
            ...
            if diff_cnt > 10000 :
                print(diff_cnt)                       
                ret_pic, frame_pic = cap.read()
                frame_pic = cv2.flip(frame_pic,1)
                cv2.imwrite('aamytest{}.jpg'.format(i),frame_pic)
                
                net_filename = 'aamytest{}.jpg'.format(i).encode()
                sock.sendto(net_filename, (UDP_IP, UDP_PORT))
                #print("Sending {} ...".format(file_name))
                f = open('aamytest{}.jpg'.format(i), "rb")
                data = f.read(buf)
                while(data):
